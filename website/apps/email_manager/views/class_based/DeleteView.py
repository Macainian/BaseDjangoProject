...
class DeleteView(BrowseView):
    def get(self, *args, **kwargs):
        ...
        if items.count() == 0:
            set_notification(self.request, "There were no emails selected to delete", ALERT_DANGER)

            raise HttpRedirectException(reverse("email_manager.browse"))
        ...
        if items_deleted_count == 0:
            set_notification(
                self.request,
                "Failed to delete " + str(items_protected_count) + " emails due to protection <br>" +
            ...
            set_notification(
                self.request,
                "Successfully deleted " + str(items_deleted_count) + " emails <br>" +
