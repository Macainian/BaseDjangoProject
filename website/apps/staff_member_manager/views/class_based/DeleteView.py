...
class DeleteView(BrowseView):
    def get(self, *args, **kwargs):
        ...
        if items.count() == 0:
            set_notification(self.request, "There were no staff members selected to delete", ALERT_DANGER)
        ...
        if items_deleted_count == 0:
            set_notification(
                self.request,
                "Failed to delete " + str(items_protected_count) + " staff members due to protection <br>" +
            ...
            set_notification(
                self.request,
                "Successfully deleted " + str(items_deleted_count) + " staff members <br>" +
